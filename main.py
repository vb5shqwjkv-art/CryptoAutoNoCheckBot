...
class Config:
    ...
    # Minimo configurato: 0.45 EUR. Il minimo EFFETTIVO per coppia sara' il piu'
    # alto tra questo valore e il minimo imposto da Kraken (cost_min / amount_min).
    min_trade_amount: float = env_float("MIN_TRADE_AMOUNT", 0.45)
    # max_trade_amount NON viene piu' usato come tetto: il massimo per trade e'
    # tutto il saldo EUR disponibile. Lasciato per compatibilita' con vecchie env.
    max_trade_amount: float = env_float("MAX_TRADE_AMOUNT", 0.0)
...
class RiskManager:
    ...
    def trade_capital(self, quote_free: float, signal_data: Optional[Signal]) -> float:
        """Massimo per trade = tutto il saldo EUR disponibile.
 
        Se il segnale e' promettente (ha superato i filtri buy a monte),
        si usa l'intero saldo libero, lasciando un piccolo margine per le
        commissioni. La verifica e l'eventuale rialzo al minimo della
        coppia avvengono in adjust_capital_for_market_limits().
        """
...
class KrakenTradingBot:
    ...
    def connect(self) -> None:
        ...
        self.telegram.send(
            ...
            "Max trade: tutto il saldo EUR disponibile\n"
    ...
    def open_trade(self, sig: Signal) -> None:
        try:
            ...
            LOGGER.info(
                "Capitale calcolato %s: %.4f %s (free=%.4f) -> usa tutto il saldo",
    ...
    def adjust_capital_for_market_limits(self, symbol: str, capital: float, quote_free: float) -> float:
        """Verifica il capitale contro i minimi di Kraken e il saldo.
 
        Logica:
        - Il minimo EFFETTIVO della coppia = il piu' alto tra:
            * cfg.min_trade_amount (0.45 EUR)
            * cost_min imposto da Kraken (+1% margine)
            * amount_min x prezzo imposto da Kraken (+1% margine)
        - Se il saldo disponibile non copre nemmeno questo minimo -> blocca (0).
        - Altrimenti compra il piu' possibile: tutto il saldo disponibile,
          ma mai sotto il minimo della coppia.
 
        Esempi:
        - capitale richiesto < minimo coppia, ma saldo >= minimo  -> compra il minimo
        - capitale richiesto > saldo                              -> compra tutto il saldo
        """
        try:
            available = max(0.0, quote_free * 0.95)
            ...
            # Minimo effettivo della coppia: parte dal minimo configurato (0.45)
            pair_min = max(self.cfg.min_trade_amount, 0.0)
 
            # Minimo di costo imposto da Kraken (in valuta quote)
            ...
            # Minimo di amount imposto da Kraken: convertito in capitale
            ...
            # Saldo insufficiente anche solo per il minimo della coppia -> blocca
            if pair_min > available:
                return 0.0
 
            # Compra il massimo possibile (tutto il saldo), mai sotto il minimo coppia.
            return max(pair_min, min(capital, available))
 
        except Exception:
            # In caso di errore imprevisto, usa il saldo disponibile in modo prudente
    ...
    def cmd_diagnostica(self) -> str:
        ...
        lines.append("  max_trade_amount: tutto il saldo EUR disponibile")
